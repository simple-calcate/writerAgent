...
function Slider({ value, onChange, min, max, step = 1, unit = '' }: {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      />
      <span className="text-[11px] text-[var(--nw-text-secondary)] w-10 text-right">{value}{unit}</span>
    </div>
  )
}
...
function Section({ title, enabled, onToggle, children }: {
  ...
  return (
    <div className="bg-[var(--nw-surface-2)]/40 rounded-lg p-3 space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-sm text-[var(--nw-text-secondary)] font-medium">{title}</span>
...
export default function VisualEffectsTab() {
  ...
  return (
    <div className="space-y-3">
      {/* Master toggle */}
      <div className="flex items-center justify-between bg-[var(--nw-surface-2)]/40 rounded-lg p-3">
        <div>
          <span className="text-sm text-[var(--nw-text-primary)] font-medium">视觉效果</span>
          <p className="text-[11px] text-[var(--nw-text-muted)] mt-0.5">开启后可使用背景、毛玻璃、光晕、雨滴等效果</p>
        </div>
      ...
      {settings.effectsEnabled && (
        <>
          ...
          <div className="bg-[var(--nw-surface-2)]/40 rounded-lg p-3 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm text-[var(--nw-text-secondary)] font-medium">背景图</span>
            ...
            {settings.backgroundImage ? (
              <>
                <div className="flex items-center gap-2">
                  <div className="w-16 h-10 rounded overflow-hidden bg-[var(--nw-surface-2)] shrink-0">
                ...
                <div>
                  <label className="text-[11px] text-[var(--nw-text-muted)] block mb-1">适应方式</label>
                      ...
                      <button
                        ...
                        className={`px-2 py-0.5 text-[11px] rounded transition-colors ${
                          settings.backgroundFit === opt.value
                            ...
                            : 'bg-[var(--nw-surface-2)] text-[var(--nw-text-secondary)] hover:text-[var(--nw-text-primary)]'
                        }`}
                      >
                        {opt.label}
                      </button>
                ...
                <div>
                  <label className="text-[11px] text-[var(--nw-text-muted)] block mb-1">缩放</label>
              ...
              <button
                ...
                className="w-full py-2 text-xs text-[var(--nw-text-secondary)] border border-dashed border-white/15 rounded hover:border-blue-500 hover:text-blue-400 transition-colors"
              >
                选择图片或视频文件
              </button>
            )}
          ...
          <Section title="毛玻璃" enabled={settings.frostedGlass} onToggle={v => settings.updateSettings({ frostedGlass: v })}>
            <div>
              <label className="text-[11px] text-[var(--nw-text-muted)] block mb-1">模糊强度</label>
          ...
          <Section title="鼠标光晕" enabled={settings.mouseGlow} onToggle={v => settings.updateSettings({ mouseGlow: v })}>
            <div>
              <label className="text-[11px] text-[var(--nw-text-muted)] block mb-1">颜色</label>
              <div className="flex gap-1.5">
                {GLOW_PRESETS.map(p => (
                  <button
                    key={p.label}
            ...
            <div>
              <label className="text-[11px] text-[var(--nw-text-muted)] block mb-1">半径</label>
              <Slider value={settings.mouseGlowRadius} onChange={v => settings.updateSettings({ mouseGlowRadius: v })} min={100} max={400} unit="px" />
            </div>
            <div>
              <label className="text-[11px] text-[var(--nw-text-muted)] block mb-1">强度</label>
          ...
          <Section title="雨滴效果" enabled={settings.rainEffect} onToggle={v => settings.updateSettings({ rainEffect: v })}>
            <div>
              <label className="text-[11px] text-[var(--nw-text-muted)] block mb-1">密度</label>
              <Slider value={settings.rainDensity} onChange={v => settings.updateSettings({ rainDensity: v })} min={50} max={300} />
            </div>
            <div>
              <label className="text-[11px] text-[var(--nw-text-muted)] block mb-1">速度</label>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 bg-[var(--nw-surface-2)] border border-white/10 rounded px-2 py-1 text-xs text-[var(--nw-text-secondary)] placeholder-gray-600"
                  readOnly
                />
                <button
                  onClick={handleDetectSteam}
                  className="px-2 py-1 text-[11px] bg-[var(--nw-surface-2)] hover:bg-white/10 text-[var(--nw-text-secondary)] rounded transition-colors shrink-0"
                >
                  自动检测
                </button>
                <button
              ...
              {settings.wallpaperEnginePath && (
                <p className="text-[10px] text-[var(--nw-text-muted)] truncate">路径：{settings.wallpaperEnginePath}</p>
              )}
              {settings.wallpaperEnginePath && (
                <button
                  onClick={handleScanWallpapers}
                  disabled={scanning}
                  className="w-full py-1.5 text-xs text-[var(--nw-text-secondary)] bg-[var(--nw-surface-2)] hover:bg-[var(--nw-surface-2)] rounded transition-colors disabled:opacity-50"
                >
                  {scanning ? '扫描中...' : '扫描壁纸'}
                </button>
              )}
                ...
                <div className="max-h-48 overflow-y-auto space-y-1">
                  {wallpapers.map(wp => (
                    <div
                      ...
                      className="flex items-center gap-2 p-2 bg-[var(--nw-surface-2)] rounded cursor-pointer hover:bg-[var(--nw-surface-2)] transition-colors"
                    >
                      <div className="w-12 h-8 rounded overflow-hidden bg-[var(--nw-surface-2)] shrink-0">
                        {wp.preview ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-[var(--nw-text-secondary)] truncate">{wp.name}</p>
                        <p className="text-[10px] text-[var(--nw-text-muted)]">{wp.type}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {wallpapers.length === 0 && settings.wallpaperEnginePath && !scanning && (
                <p className="text-[11px] text-[var(--nw-text-muted)] text-center py-2">未找到壁纸，请检查路径是否正确</p>
              )}
            </div>
          </Section>
        </>
