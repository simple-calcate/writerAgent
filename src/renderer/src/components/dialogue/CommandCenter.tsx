...
export default function CommandCenter({
  ...
  if (!badge) return <div className="border-t border-white/5 p-3 bg-[--surface-1]">{children}</div>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-1.5">
          {runState === 'running' && onPause && (
            <button onClick={onPause} className="text-[10px] text-[--text-muted] hover:text-yellow-400 px-2 py-0.5 rounded hover:bg-yellow-500/10 transition-colors">
              暂停
            </button>
          )}
          {(runState === 'paused' || runState === 'done') && onResume && (
            <button onClick={onResume} className="text-[10px] text-[--text-muted] hover:text-emerald-400 px-2 py-0.5 rounded hover:bg-emerald-500/10 transition-colors">
              继续
            </button>
          )}
          {runState === 'done' && onReplay && (
            <button onClick={onReplay} className="text-[10px] text-[--text-muted] hover:text-blue-400 px-2 py-0.5 rounded hover:bg-blue-500/10 transition-colors">
              回放
            </button>
          )}
        </div>
