    ...
    <div className="rounded-md bg-[--surface-1] shadow-[0_0_0_1px_rgba(255,255,255,0.04)]">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span>共享记忆</span>
          <span className="text-[10px] text-[--text-muted]">{totalItems} 条</span>
        </div>
      ...
      {!collapsed && (
        <div className="px-3 pb-2 space-y-2">
          {/* Facts */}
          <div>
            <p className="text-[10px] text-[--text-muted] mb-1">事实</p>
            {memory.facts.length > 0 ? (
              <div className="space-y-0.5">
                {memory.facts.slice(-5).map((f, i) => (
                  <div key={i} className="text-[11px] text-[--text-secondary] px-2 py-0.5 rounded bg-[--surface-2] truncate">
                    {f}
                  </div>
                ))}
                {memory.facts.length > 5 && (
                  <p className="text-[9px] text-[--text-muted] pl-2">+{memory.facts.length - 5} 更多</p>
                )}
              </div>
            ) : (
