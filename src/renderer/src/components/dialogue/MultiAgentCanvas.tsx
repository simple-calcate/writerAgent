  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[11px] text-[--text-primary] truncate">{node.label}</span>
      </div>
      {expanded && node.output && (
        <div className="text-[10px] text-[--text-muted] mt-1 max-h-16 overflow-y-auto">
...
function AgentLane({ agent }: { agent: AgentInstance }) {
  const role = ROLE_LABELS[agent.role] || { label: agent.role, icon: '•', color: 'text-[--text-muted]' }
    ...
    <div className={`border-b border-white/5 py-2 px-3 relative ${isActive ? 'bg-white/[0.01]' : ''}`}>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-thin">
        {agent.graph.nodes.length === 0 ? (
          <span className="text-[10px] text-[--text-muted] italic">等待中...</span>
        ) : (
