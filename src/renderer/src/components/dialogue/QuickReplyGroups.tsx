  ...
  return (
    <div className="space-y-2">
      {groups.map((group, gi) => (
        <div key={gi}>
          {group.question && (
            <p className="text-[11px] text-[--nw-text-muted] mb-1.5 pl-2.5 border-l-2 border-[--nw-border]">
              {group.question}
            </p>
          )}
          <div className="flex flex-wrap gap-1.5">
            {group.options.map((reply, ri) => (
              <button
                ...
                className="px-2.5 py-1.5 text-[11px] bg-[--nw-surface-2] hover:bg-[--nw-surface-1]
                           text-[--nw-text-secondary] hover:text-[--nw-text-primary]
              ...
              >
                <span className="text-[--nw-text-muted] mr-1.5">{ri + 1}.</span>
                {reply.value}
              </button>
            ))}
          </div>
