  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2">
        ...
        {text.length > 0 && <span className="text-[var(--nw-text-muted)] text-[10px]">{text.length} 字</span>}
        {onCancel && (
          <button
            onClick={onCancel}
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      >
        ...
        <span className="text-[var(--nw-text-muted)] text-[10px]">{text.length} 字</span>
      ...
      {expanded && text.length > 0 && (
        <div ref={scrollRef} className="mt-1.5 max-h-36 overflow-y-auto">
          ...
          <div className="text-[11px] leading-[1.7] text-[var(--nw-text-muted)] opacity-60 italic border-l border-[var(--nw-border)] pl-2 font-mono whitespace-pre-wrap break-all">
            {text}
          </div>
        </div>
      )}
